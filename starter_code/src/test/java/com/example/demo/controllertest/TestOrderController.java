...
class TestOrderController {
	...
	public void initUserCreation() throws Exception {
		...
		userRequest.setUsername("LohitakshTestFour");
		...
		cartRequest.setUsername("LohitakshTestFour");
	...
	public void testOrderCreationAndHistory() throws Exception {
		...
		MvcResult result = mockMvc.perform(MockMvcRequestBuilders.post("/api/order/submit/{username}", "LohitakshTestFour")
				.accept(MediaType.APPLICATION_JSON).header("Authorization", request.getParameter("Authorization")))
				.andExpect(status().isOk()).andReturn();
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/api/order/submit/{username}", "LohitakshTestFour")
		...
		MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.get("/api/order/history/{username}", "LohitakshTestFour")
				.accept(MediaType.APPLICATION_JSON).header("Authorization", request.getParameter("Authorization")))
				.andExpect(status().isOk()).andReturn();
		...
		mockMvc.perform(MockMvcRequestBuilders.get("/api/order/history/{username}", "LohitakshTestFour")
				.accept(MediaType.APPLICATION_JSON)).andExpect(status().isUnauthorized());
	}
}
